...
function BaseInteractionExt:can_select(player)
	if mic_is_enabled and self.tweak_data == "hostage_move" and not managers.groupai:state():whisper_mode() then
	...
	if mic_is_enabled and self.tweak_data == "hostage_convert" and (managers.groupai:state():whisper_mode() or managers.player:chk_minion_limit_reached()) then
